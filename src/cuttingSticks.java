...
public class cuttingSticks {
     ...
     */
    static int cutting_stickes_cnt(String answers){
        int result = 0;
        int continue_chk = 0;
        Stack<Character> stick_stack = new Stack<>();

        for(int i=0 ; i < answers.length(); i++){
            if(answers.charAt(i) == '(') stick_stack.push(answers.charAt(i));
            else {
                stick_stack.pop();
                // empty 설정 시 마지막 괄호 미카운트로 오류
                if(answers.charAt(i-1) == '(') result += stick_stack.size();
                else result++;
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(cutting_stickes_cnt(sc.nextLine()));
    }
}
